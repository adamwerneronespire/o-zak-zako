...
* Use the forms here only if they satisfy all of your requirements



* and are acceptable in every respect!



* !!!   MODIFYING THE FORMS IS PROHIBITED !!!!
...
*  --> $pf_status  name of the form that sets the list status - can be blank if
...
*  --> $user_command name of the form that provides list interactivity - can be



*                    blank if the standard behavior is sufficient. The specified form must be
...
* Initializing and populating list values
...
*        Same as the standard ALV list, but the display is in GRID
...
FORM COMMON_GS_LAYOUT_BUILD USING $GS_LAYOUT TYPE SLIS_LAYOUT_ALV.
  ...
  $GS_LAYOUT-COLWIDTH_OPTIMIZE   = 'X'."Optimize columns
...
*  $GS_LAYOUT-TOTALS_BEFORE_ITEMS = 'X'."Totals before the items
...
*   $gs_layout-NO_MIN_LINESIZE = 'X'. " line size = width of the list
  ...
  $GS_LAYOUT-GET_SELINFOS = 'X'. "Read the selections



*  $gs_LAYOUT-GROUP_CHANGE_EDIT = 'X'. "Subtotal formatting can be modified
...
*  $gs_layout-group_change_edit = 'X'.  "The user can change it,



*                                       "to place subtotals on a new page during sorting



*                                       "or place them on a new page



*                                       "or separate them with underlining
...
*      -->$INCLNAME    Name of the include where the internal table is declared



*                      Only fill it if the internal table is not defined as a DDIC structure!!
...
*           Examples for additional ALV-related form calls
...
*       For summarization: to define the sequence
...
*-- Field order difference -> place the given field in the first column
