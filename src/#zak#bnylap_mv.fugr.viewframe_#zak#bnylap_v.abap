*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_/ZAK/BNYLAP_V
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_/ZAK/BNYLAP_V        .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_/ZAK/BNYLAP_V'
         EXPORTING
              FCODE          = READ
...
EXTRACT = /ZAK/BNYLAP_V_EXTRACT
TOTAL = /ZAK/BNYLAP_V_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_/ZAK/BNYLAP_V'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_/ZAK/BNYLAP_V-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = /ZAK/BNYLAP_V_EXTRACT
TOTAL = /ZAK/BNYLAP_V_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_/ZAK/BNYLAP_V-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_/ZAK/BNYLAP_V'
...
TOTAL = /ZAK/BNYLAP_V_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_/ZAK/BNYLAP_V-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_/ZAK/BNYLAP_V-UPD_FLAG NE SPACE.
