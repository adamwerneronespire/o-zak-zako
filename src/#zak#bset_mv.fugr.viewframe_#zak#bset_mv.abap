*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_/ZAK/BSET_MV
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_/ZAK/BSET_MV         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_/ZAK/BSET_MV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = /ZAK/BSET_MV_EXTRACT
TOTAL = /ZAK/BSET_MV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_/ZAK/BSET_MV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_/ZAK/BSET_MV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = /ZAK/BSET_MV_EXTRACT
TOTAL = /ZAK/BSET_MV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_/ZAK/BSET_MV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_/ZAK/BSET_MV'
...
TOTAL = /ZAK/BSET_MV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_/ZAK/BSET_MV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_/ZAK/BSET_MV-UPD_FLAG NE SPACE.
