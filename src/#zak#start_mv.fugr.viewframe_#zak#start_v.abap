*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_/ZAK/START_V
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_/ZAK/START_V         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_/ZAK/START_V'
         EXPORTING
              FCODE          = READ
...
EXTRACT = /ZAK/START_V_EXTRACT
TOTAL = /ZAK/START_V_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_/ZAK/START_V'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_/ZAK/START_V-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = /ZAK/START_V_EXTRACT
TOTAL = /ZAK/START_V_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_/ZAK/START_V-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_/ZAK/START_V'
...
TOTAL = /ZAK/START_V_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_/ZAK/START_V-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_/ZAK/START_V-UPD_FLAG NE SPACE.
