*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_/ZAK/UREPI_MV
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_/ZAK/UREPI_MV        .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_/ZAK/UREPI_MV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = /ZAK/UREPI_MV_EXTRACT
TOTAL = /ZAK/UREPI_MV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_/ZAK/UREPI_MV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_/ZAK/UREPI_MV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = /ZAK/UREPI_MV_EXTRACT
TOTAL = /ZAK/UREPI_MV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_/ZAK/UREPI_MV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_/ZAK/UREPI_MV'
...
TOTAL = /ZAK/UREPI_MV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_/ZAK/UREPI_MV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_/ZAK/UREPI_MV-UPD_FLAG NE SPACE.
